    ...
    return (
      <DashboardLayout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
          ...
          <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">Cargando tu ecosistema...</p>
        </div>
      </DashboardLayout>
    );
  }
    ...
    <DashboardLayout>
      <div className="max-w-6xl mx-auto space-y-12 pb-20">
        ...
        <section className="space-y-8">
          <div>
            <h1 className="text-4xl font-black text-slate-900 tracking-tight">Mi Plan y Capacidad</h1>
            <p className="text-slate-500 font-medium">Gestiona los límites y recursos de tu academia.</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <Card className="md:col-span-1 border-none shadow-xl rounded-[2.5rem] bg-indigo-600 text-white overflow-hidden relative group">
              ...
              <CardContent className="p-8 space-y-6 relative z-10">
                ...
                <div>
                  ...
                  <p className="text-indigo-100/70 text-sm font-bold uppercase tracking-tighter">
                    {daysLeft > 0 ? `Vence en ${daysLeft} días` : 'Expirado o sin vigencia'}
                  </p>
                </div>
                <div className="pt-4 flex items-center gap-2">
                  <div className="h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
                  <span className="text-xs font-bold text-indigo-100">Sincronizado con Producción</span>
                </div>
              </CardContent>
            </Card>
              ...
              <Card className="border-none shadow-xl rounded-[2.5rem] bg-white p-8 space-y-6">
                <div className="flex justify-between items-start">
                  <div className="space-y-1">
                    ...
                    <h3 className="text-3xl font-black text-slate-900">{courseCount} <span className="text-sm text-slate-400 font-bold">/ {limits.maxCourses === -1 ? '∞' : limits.maxCourses}</span></h3>
                  </div>
                ...
                <div className="space-y-2">
                  <Progress value={courseProgress} className="h-2 bg-slate-100" indicatorClassName="bg-indigo-600" />
                  <p className="text-[10px] font-bold text-right text-slate-400 uppercase tracking-widest">
              ...
              <Card className="border-none shadow-xl rounded-[2.5rem] bg-white p-8 space-y-6">
                <div className="flex justify-between items-start">
                  <div className="space-y-1">
                    ...
                    <h3 className="text-3xl font-black text-slate-900">{studentCount} <span className="text-sm text-slate-400 font-bold">/ {limits.maxStudents === -1 ? '∞' : limits.maxStudents}</span></h3>
                  </div>
                ...
                <div className="space-y-2">
                  <Progress value={studentProgress} className="h-2 bg-slate-100" indicatorClassName="bg-emerald-500" />
                  <p className="text-[10px] font-bold text-right text-slate-400 uppercase tracking-widest">
                ...
                <div className="md:col-span-2 flex items-center gap-12">
                  ...
                  <div className="flex-1 space-y-4">
                    <div className="flex justify-between text-[10px] font-black uppercase tracking-widest">
                      <span className="text-slate-400">Progreso de Consumo</span>
                      <span className="text-indigo-600">Total: {sub?.aiQuotas?.totalCredits || 0}</span>
                    </div>
                    <Progress 
            ...
            <div className="md:col-span-3 grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-4">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Invitaciones</p>
                  <p className="text-lg font-black text-slate-900">{sub?.invitationsPerCourse || 0} <span className="text-[10px] text-slate-400">/ curso</span></p>
                </div>
              </div>
              ...
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-4">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Branding</p>
                  <p className="text-lg font-black text-slate-900">{sub?.limits?.hasCustomBranding ? 'Habilitado' : 'Estándar'}</p>
                </div>
              </div>
              ...
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-4">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">IA Premium</p>
                  <p className="text-lg font-black text-slate-900">{sub?.hasPremiumAI ? 'Activa' : 'No'}</p>
                </div>
              </div>
              ...
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-4">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Fin de Abono</p>
                  <p className="text-lg font-black text-slate-900">{endD ? format(endD, 'dd/MM/yy') : '---'}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-16">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
                    <Zap className="h-6 w-6 text-amber-500" /> Recargar Créditos Extra
                  </h2>
                  <p className="text-slate-500 font-medium">Añade potencia de fuego a tu plan {currentPlan.name} sin cambiar de suscripción.</p>
                </div>
              </div>
                  ...
                  .map((opt: any, idx: number) => (
                    ...
                    >
                      ...
                      <div className="space-y-6 relative z-10">
                        <div className="space-y-1">
                          <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">Pack de Créditos</p>
                          <h4 className="text-3xl font-black text-slate-900">+{opt.credits}</h4>
                        </div>
          ...
          <section className="space-y-8">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
                  <Rocket className="h-6 w-6 text-indigo-500" /> Mejorar tu Plan
                </h2>
                <p className="text-slate-500 font-medium">Escala tus límites y desbloquea herramientas de IA avanzada.</p>
              </div>
            </div>
                      ...
                      <ul className="space-y-4 mb-10 flex-1">
                        ...
                        {plan.hasPremiumAI && (
                          <li className="flex items-start gap-3">
                            ...
                            <span className="text-sm font-bold text-amber-600">IA Premium Habilitada</span>
                          </li>
                        )}
                      </ul>
        ...
        <section className="bg-white rounded-[3rem] p-10 shadow-xl border border-slate-100 flex flex-col md:flex-row items-center gap-10">
          ...
          <div className="space-y-2">
            <h4 className="text-xl font-black text-slate-900">¿Necesitas un plan a medida?</h4>
            <p className="text-slate-500 font-medium">Si tu academia supera los 2,000 alumnos o requieres integraciones personalizadas, contacta a nuestro equipo de soporte para un plan Enterprise exclusivo.</p>
          </div>
            ...
            {selectedPlan?.isCreditPack ? (
              <div className="bg-amber-50 border border-amber-100 rounded-2xl p-5">
                <div className="flex justify-between items-center">
                  <div>
                    <p className="text-sm font-black text-amber-900">Total a Pagar</p>
                    <p className="text-xs text-amber-700 font-medium">Carga inmediata de saldo</p>
                  </div>
                  <span className="text-2xl font-black text-amber-900">${selectedPlan.packPrice}</span>
                </div>
              </div>
            ) : upgradeInfo && (
              <div className="bg-amber-50 border border-amber-100 rounded-2xl p-5 space-y-3">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-black text-amber-900">Prorrateo de Mejora</p>
                ...
                <div className="pt-3 border-t border-amber-200 flex justify-between items-center">
                  <span className="text-[10px] font-black uppercase text-amber-600 tracking-widest">Diferencial:</span>
                ...
                {paymentMethods.map(method => (
                  <button
                    ...
                    className={cn(
                      ...
                      selectedMethodId === method.id ? "border-indigo-600 bg-indigo-50/50" : "border-slate-100"
                    )}
                  >
                    <div className="flex items-center gap-3">
