...
import { generateIdentityDesign as generateIdentityDesignAI, generateIdentityDesignBatch } from '@/ai/flows/generate-identity-design';
...
export function useIdentityDesign() {
  ...
  const [identityDesign, setIdentityDesign] = useState<IdentityDesign | null>(null);
  const [isDesignApproved, setIsDesignApproved] = useState(false);

  // Generar diseño de identidad (galería de 5)
  const generateIdentityDesign = useCallback(async (directives: string) => {
    setIsDesigning(true);
    setDesignProgress({ current: 0, total: 100, label: "Enviando directivas a IA..." });

    try {
      setDesignProgress({ current: 20, total: 100, label: "IA analizando requisitos..." });
      await new Promise(resolve => setTimeout(resolve, 1000));

      setDesignProgress({ current: 40, total: 100, label: "Generando 5 propuestas..." });
      await new Promise(resolve => setTimeout(resolve, 1500));

      setDesignProgress({ current: 60, total: 100, label: "Creando paletas de colores..." });
      await new Promise(resolve => setTimeout(resolve, 1500));

      setDesignProgress({ current: 80, total: 100, label: "Seleccionando tipografías..." });
      await new Promise(resolve => setTimeout(resolve, 1000));
      ...
      setDesignProgress({ current: 100, total: 100, label: "Diseños completados" });
      ...
      toast({
        ...
        description: "La IA ha creado 5 diseños únicos. Navega entre ellos con las flechas."
      });
  ...
  const navigateDesign = useCallback((direction: 'next' | 'prev') => {
    if (designGallery.length === 0) return;

    const newIndex = direction === 'next' 
      ? (currentDesignIndex + 1) % designGallery.length
      : (currentDesignIndex - 1 + designGallery.length) % designGallery.length;
  ...
  const hasNextDesign = useCallback(() => {
    return designGallery.length > 0 && currentDesignIndex < designGallery.length - 1;
  }, [designGallery, currentDesignIndex]);
  ...
  const updateIdentityDesign = useCallback(async (updates: Partial<IdentityDesign>) => {
    if (!identityDesign) return null;

    setIsDesigning(true);
    setDesignProgress({ current: 0, total: 100, label: "Aplicando ajustes..." });
    ...
    try {
      ...
      const updatedDesign: IdentityDesign = {
        ...
        ...updates,
        rationale: {
          colors: updates.rationale?.colors || identityDesign.rationale.colors,
          typography: updates.rationale?.typography || identityDesign.rationale.typography,
          overall: "Diseño actualizado según tus preferencias. Se han mantenido los principios fundamentales de la identidad visual."
        }
      };
      ...
      setDesignProgress({ current: 100, total: 100, label: "Ajustes aplicados" });
  ...
  const approveDesign = useCallback(() => {
    ...
    toast({
      ...
      description: "Ahora puedes generar los planos omnicanal con esta identidad visual."
    });
  }, [toast]);
