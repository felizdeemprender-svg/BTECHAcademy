...
export function useCollections(profile: any) {
  ...
  // Query para colecciones del usuario
  const collectionsQuery = useMemoFirebase(() => {
    if (!profile?.uid) return null;
    return query(collection(db, 'templateCollections'), where('ownerId', '==', profile.uid));
  }, [db, profile?.uid]);

  const { data: rawCollections, isLoading: isQueryLoading } = useCollection(collectionsQuery);
  ...
  const createCollection = useCallback(async (name: string, directives: string) => {
  ...
  const updateCollection = useCallback(async (collectionId: string, updates: Partial<TemplateCollection>) => {
    setIsLoading(true);
    try {
