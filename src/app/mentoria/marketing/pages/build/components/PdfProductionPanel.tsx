...
export function PdfProductionPanel({
  ...
  return (
    <div className="p-8 rounded-lg border-2 border-dashed space-y-8 bg-muted/40 backdrop-blur-md" 
         style={{ borderColor: isGenerating ? '#10b981' : 'var(--border)' }}>
      ...
      <div className="flex items-center gap-4 mb-2">
        ...
        <div>
          <h3 className="text-lg font-bold text-foreground">Producción de Documento LinkedIn</h3>
      ...
      <div className="p-6 bg-white rounded-3xl border border-border space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="p-3 bg-white/60 rounded-xl border border-border text-center">
            <p className="text-[10px] text-muted-foreground font-bold uppercase">Páginas</p>
            <p className="text-xl font-black text-foreground">{s.slides?.length || 0}</p>
          </div>
          <div className="p-3 bg-white/60 rounded-xl border border-border text-center">
          ...
          <div className="p-3 bg-white/60 rounded-xl border border-border text-center">
            ...
            <p className="text-xs font-bold text-muted-foreground">Manual</p>
          </div>
        </div>
      </div>
      ...
      {!pdfUrl ? (
        ...
        <div className="space-y-4">
          <div className="flex flex-col md:flex-row gap-3">
            ...
            <Button 
              className="flex-1 h-16 rounded-2xl bg-foreground hover:bg-foreground/90 text-white font-black uppercase tracking-widest text-sm gap-3 border border-border transition-all active:scale-95"
              onClick={() => onGeneratePdf(s, sIdx)}
            >
              Regenerar
            </Button>
