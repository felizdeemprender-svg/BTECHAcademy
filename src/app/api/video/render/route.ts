...
async function runFfmpeg(args: string[], label?: string): Promise<void> {
  ...
  // 1. PRIORIDAD M├üXIMA: Usar el binario personalizado con soporte drawtext/libfreetype
  ...
  for (const p of customBinaryPaths) {
    if (fs.existsSync(p)) {
      ffmpegPath = p;
      console.log(`[FFmpeg:Path] Ô£à Usando binario PERSONALIZADO (con drawtext): ${p}`);
      break;
    }
  }
  
  // 2. Fallback: ffmpeg-static (sin drawtext ÔÇö ├║ltimo recurso)
  if (!ffmpegPath) {
    const fallbackPaths = [
    ...
    for (const p of fallbackPaths) {
      if (p && fs.existsSync(p)) {
        ffmpegPath = p;
        console.warn(`[FFmpeg:Path] ÔÜá´©Å Usando ffmpeg-static (SIN drawtext): ${p}`);
        break;
      }
    }
  }

  // 3. ├Ültimo recurso: comando global
  if (!ffmpegPath) {
    ffmpegPath = exeName;
    console.warn(`[FFmpeg:Path] ÔÜá´©Å Usando comando global: ${exeName}`);
  }

  // LOG DE DIAGN├ôSTICO (Solo visible en servidor)
  console.log(`[FFmpeg:Path] Usando binario en: ${ffmpegPath} (Existe: ${fs.existsSync(ffmpegPath)})`);

  // 4. Asegurar permisos de ejecuci├│n en Linux
  ...
  return new Promise((resolve, reject) => {
    // B├║squeda robusta de la carpeta de fuentes
    ...
    proc.on('close', (code: number) => {
      if (code === 0) resolve();
        ...
        // Mostrar m├ís l├¡neas de error para diagn├│stico y separar con saltos de l├¡nea reales
        ...
        const finalErr = `FFmpeg fall├│ (c├│digo ${code}). Detalles:\n${errLines}`;
...
// L├ôGICA DE TEXTO NIVEL DIOS (Capas, Resplandores y ├üreas Seguras)
function getDrawtextFilter(adnConfig: any, scene: Scene, brandColor: string, width: number, height: number, textFilePath: string) {
  ...
  if (fs.existsSync(fontAbsPath)) {
    ...
    console.warn(`[DrawText] ÔÜá´©Å Fuente ${fontFile} NO encontrada. Usando fallback Inter-Black.ttf`);
    resolvedFontPath = fallbackPath;
  }

  // Si ni la fuente deseada ni el fallback existen, no podemos renderizar texto
  if (!resolvedFontPath) {
    console.error(`[DrawText] ­ƒÜ¿ CR├ìTICO: Ninguna fuente encontrada en ${fontsDir}. Omitiendo drawtext.`);
    return null;
  }
  ...
  // Traducci├│n de alias sem├ínticos a expresiones matem├íticas de FFmpeg
  ...
  // Animaciones Cin├®ticas (Modifican posici├│n)
  ...
  // Usamos comillas simples para evitar que FFmpeg malinterprete caracteres en las rutas.
  ...
  // CONSTRUCCI├ôN DE CADENA DE FILTROS (M├║ltiples capas para efecto Glow/Shadow/Stroke)
  let filter = '';
  ...
  // 2. Capa de Sombra 3D (M├║ltiples offsets)
...
// FUNCI├ôN DE POST-PRODUCCI├ôN PROFESIONAL
function getPostProductionFilters(adnConfig: any, segmentLabel: string | undefined) {
  ...
  // LUT Support (Simulaci├│n con EQ si no hay archivo, pero preparado para .cube)
  if (fx.colorGrade_lut) {
     filters.push('eq=contrast=1.1:saturation=1.2'); // Simulaci├│n
  }
...
export async function POST(req: NextRequest) {
  // FIX: En entornos Serverless/Cloud Run (Firebase App Hosting), el ├║nico
  ...
  try {
    ...
    // 2. GENERACI├ôN DE VOCES (TTS)
    const ttsApiKey = process.env.GOOGLE_TTS_API_KEY || process.env.GOOGLE_API_KEY;
    const voiceClips: (string | null)[] = new Array(scenes.length).fill(null);
      ...
      if (voiceover && !isCarousel) {
        ...
        for (let i = 0; i < scenes.length; i++) {
          ...
          if (s.voiceover || s.text) {
            ...
            voiceClips[i] = filePath; // Guardar por índice exacto
            try {
    ...
    for (let i = 0; i < scenes.length; i++) {
      ...
      // L├ôGICA DE PROCESAMIENTO DE TEXTO (Wrap y Animaci├│n)
      const segment = scene.segment_label || 'default';
      ...
      // L├ôGICA RESPONSIVA MEJORADA: Escalar texto al ancho y prever auto-achique
      ...
      // Auto-escalado hacia abajo si la palabra m├ís larga romper├¡a la pantalla
      ...
      activeRule.fontsize = responsiveFontsize; // Para que getDrawtextFilter use el tama├▒o redimensionado

      // Calcular maxChars din├ímicamente sin m├¡nimos fijos que causan desbordamiento
      ...
      // NOTA: El efecto typewriter no est├í soportado nativamente por textfile+expansion 
      // en formato %{eif} ya que genera errores o imprime caracteres no v├ílidos.
      // En el futuro requerir├í subt├¡tulos .ass. Por ahora, forzamos texto completo.
      let fileContent = displayText;
      await writeFile(textFile, fileContent, 'utf8');

      // PASO CR├ìTICO: Convertir JPG a PNG para bypass de MJPEG/yuvj420p que crashea en Windows
      ...
      // FFmpeg crashea si le pasamos un textfile vac├¡o.
      ...
      // Movimiento de C├ímara: Jerarqu├¡a ADN (Segmento -> Default -> Global)
    ...
    if (isCarousel) {
      ...
      for (let i = 0; i < sceneClips.length; i++) {
        ...
        const vClip = voiceClips[i]; // Podr├¡a no existir
      ...
      // 4. MODO VIDEO ├ÜNICO: Ensamblaje Concatenado
      ...
      if (!finalVoicePath && voiceClips.length > 0) {
        ...
        await writeFile(voiceConcatFile, voiceClips.filter(Boolean).map(f => `file '${f!.replace(/\\/g, '/')}'`).join('\n'));
    ...
    console.error("­ƒôø critical error:", err.message);
