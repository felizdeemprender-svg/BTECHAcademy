...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // 3. RENDERIZADO DE PLACAS (Secuencial para estabilidad, con 2 hilos por proceso)
    ...
    for (let i = 0; i < scenes.length; i++) {
      ...
      let imgData: Buffer;
      try {
        // Resolución de URL más robusta para entornos serverless
        let urlToFetch = safeUrl;
        if (safeUrl.startsWith('/')) {
          const host = req.headers.get('host') || 'localhost:9002';
          const protocol = host.includes('localhost') ? 'http' : 'https';
          urlToFetch = `${protocol}://${host}${safeUrl}`;
        }
      ...
      console.log(`[Render:Plate${i}] Procesando secuencial... text='${(scene.text||'').substring(0,40)}'`);
      
      // Usamos el JPG directamente en Linux para ahorrar tiempo de conversión
      const inputArgs = ['-loop', '1', '-framerate', '30', '-i', assembledImg.replace(/\\/g, '/')];
      await runFfmpeg([
        ...inputArgs,
        '-t', String(sceneDuration.toFixed(2)),
        '-vf', finalFilters.join(','),
        '-c:v', 'libx264', '-preset', 'ultrafast', '-threads', '2', '-pix_fmt', 'yuv420p', '-an', '-y', outputClip.replace(/\\/g, '/')
      ], `plate_${i}`);
