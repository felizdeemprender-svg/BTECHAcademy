...
const CustomAlert = ({ variant, children, className }: any) => (
  <div className={`p-4 rounded-xl border-2 flex gap-3 ${variant === 'destructive' ? 'bg-danger/10 border-danger/20 text-danger' : 'bg-blue-50 border-blue-200 text-blue-900'} ${className}`}>
    {children}
  </div>
);
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#f8fafc] p-6 text-center space-y-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Conectando con el entorno seguro de Evolución...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#f8fafc] p-6">
      <div className="w-full max-w-[440px] space-y-8 animate-in fade-in zoom-in duration-500">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-3xl font-headline font-bold text-foreground tracking-tight">Evolución Académica</h1>
          <p className="text-muted-foreground font-medium">Plataforma Institucional de Formación</p>
        </div>
        ...
        <Card className="rounded-lg overflow-hidden bg-white">
          <CardContent className="p-8 pt-10 space-y-6">
              ...
              <CustomAlert variant="destructive">
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    <code className="bg-white p-2 rounded border border-danger/20 flex-1 font-mono break-all text-[10px] text-danger">
                      {unauthorizedDomain}
                    </code>
            ...
            {popupLostError && (
              <div className="space-y-4 animate-in slide-in-from-top-2">
                <CustomAlert variant="destructive" className="bg-warn/10 border-warn/20 text-warn">
                  <div className="space-y-3">
                    <p className="font-bold flex items-center gap-2"><Cookie className="h-4 w-4 text-warn" /> Error de Comunicación (401)</p>
                ...
                <div className="grid gap-2">
                  <Button variant="default" className="w-full h-14 rounded-xl font-bold bg-warn hover:bg-warn shadow-lg flex items-center justify-center gap-2 text-white" onClick={handleGoogleRedirectLogin}>
            ...
            {!popupLostError && !showPopupWarning && (
              <div className="space-y-6">
                <div className="text-center space-y-2">
                  <h3 className="font-bold text-foreground">Acceso a la plataforma</h3>
                ...
                <div className="space-y-3">
                  <Button variant="outline" className="w-full h-14 rounded-xl text-base font-bold shadow-sm flex items-center justify-center gap-3 border-border hover:bg-muted transition-all hover:scale-[1.02]" onClick={handleGoogleLogin} disabled={loading}>
                    <Globe className="h-5 w-5 text-accent" /> Continuar con Google
                  </Button>
                  <Button variant="ghost" className="w-full h-10 rounded-xl text-[10px] font-bold text-muted-foreground uppercase tracking-widest hover:bg-muted" onClick={handleGoogleRedirectLogin} disabled={loading}>
          ...
          <CardFooter className="bg-muted/50 p-6 border-t border-muted">
            <div className="flex flex-col items-center gap-2 mx-auto">
              <div className="flex items-center gap-2">
                <ShieldCheck className="h-4 w-4 text-muted-foreground" />
                <p className="text-[10px] text-muted-foreground font-bold tracking-widest uppercase">Entorno Seguro Evolución</p>
              </div>
              <p className="text-[9px] text-muted-foreground font-medium max-w-[200px] text-center">Solo usuarios previamente autorizados por la institución pueden acceder a este entorno.</p>
            </div>
          </CardFooter>
        </Card>
      </div>
